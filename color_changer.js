...
function getInverse(theString) {
  ...
  c=c1+c2;
  var newInverseColor=DecToHex(255-a)+""+DecToHex(255-b)+""+DecToHex(255-c);
  return newInverseColor;
}
...
function initAddButton() {
  ...
  var inverseColor = inverseHex(colors[i]);
  btn.innerHTML = colors[i];
  btn.setAttribute('onclick','background(\''+colors[i]+'\')');
  btn.style.backgroundColor = colors[i];
  btn.style.color = '#' + inverseColor;
...
for (var i = 0; i < colorList.length; i++) {
  ...
  optionAdd.textContent = color;
  optionAdd.value = color;
...
function addButton() {
  ...
  } else if (colors.indexOf(newColor) !== -1){
...
function inverseHex(whatevColor) {
  var hex;
  for (var i = 0; i < colorList.length; i++) {
    if(colorList[i] === whatevColor) {
      hex = hexList[i];
      break;
    }
  }
  var inverseColor = getInverse(hex);
  return inverseColor;
}

function initHexList() {
  hexList = ['F0F8FF',
'FAEBD7',
'00FFFF',
'7FFFD4',
